...
- Resource Atlas: remains idle above the fold, loads on approach or a direct resource anchor, and reports `1007 of 1007 resources | showing 8` on a fresh mobile load.
- Resource interactions: the Model loop-layer filter returns 31 of 1007 resources (30 papers plus Awesome Loop Models), paginates eight at a time on mobile, and renders Loopie and LoopWM with their 2026 arXiv records and adjacent-scope labels.
...
- Source audit: 1007 rows checked; 943 public sources reachable, 13 access-restricted, 51 repository-native, and 0 broken or unreachable as of 2026-09-09 UTC.
- Data and copy: the website, README, translations, social preview source, release copy, and Hugging Face card all report 1007 resources, 22 patterns, 22 contracts, 8 runtime starters, and version 0.9.0. The dataset exposes 50 fields, including `loop_layer` and `scope_fit`.
