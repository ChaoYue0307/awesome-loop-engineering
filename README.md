...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/resources-483-a78bfa" alt="resources">
...
- 483 curated resource rows with tabular exports
...
- 🧰 **Tool** [LoongFlow (Baidu)](https://github.com/baidu-baige/LoongFlow) - Baidu's open-source expert-grade agent framework explicitly built for Loop Engineering, where a Plan-Execute-Summary loop with structured experiential memory lets agents plan, execute, reflect, and evolve across software-engineering, math, and ML tasks (Apache-2.0, on PyPI, paper: arXiv 2512.24077).
- 🧰 **Tool** [cc10x](https://github.com/romiluz13/cc10x) - Claude Code plugin (v12.5.0, 150 stars) that routes work through one router, nine specialist agents, sixteen skills, and four workflows, enforcing fail-closed verification and test-honesty gates and writing each workflow's intent, evidence, and verdicts to durable .cc10x/ disk artifacts so resume and review survive context compaction.
...
- 📄 **Paper** [Physics-Audited Agentic Discovery in Scientific Machine Learning](https://arxiv.org/abs/2607.07379) - PA-SciML, a verification-first workflow for agentic model discovery in scientific ML: fixes the scoring evaluator before search, derives reviewable machine-checkable physics requirements (boundary conditions, superposition, stiffness scaling, causality), audits every trained candidate's predicted fields against them, and separately searches prescribed input ranges for high-violation cases, reporting a surrogate as verified only under the stated checks, a domain-specific but clean instance of verification-gated agentic search.
...
- 📄 **Paper** [Failure as a Process: An Anatomy of CLI Coding Agent Trajectories](https://arxiv.org/abs/2607.09510) - Empirical anatomy of 3,843 CLI coding-agent trajectories across seven models and three scaffolds (OpenHands, MiniSWE, Terminus2), with 1,794 fully annotated over 63,000+ manually reviewed steps, modeling failure as a temporal process of onset, evolution, and recovery and finding failures are dominated by epistemic errors that begin within the first few steps yet stay undetected until recovery is impossible, an argument for in-loop validation and intervention over final-outcome evaluation.
...
- 📄 **Paper** [Token-Flow Firewall: Semantic Runtime Auditing for Persistent AI Agents](https://arxiv.org/abs/2607.08395) - Proposes TokenWall, a runtime firewall that audits semantic flows in long-lived agents, memory updates, tool arguments, and inter-component messages, before they reach privileged sinks, reducing attack success rates to 12.5% while maintaining a 97.4% benign pass rate with 0.69s added latency.
...
- 📄 **Paper** [TRACE: A Two-Channel Robust Attribution Watermark via Complementary Embeddings for LLM-Agent Trajectories](https://arxiv.org/abs/2607.08400) - Embeds a two-channel attribution watermark in LLM-agent trajectory logs, one channel keyed on content for deletion resistance, one on log structure for rewrite resistance, so provenance survives an adversary with full read/write access, with detection scores near z = 100 on long-horizon ToolBench and ALFWorld trajectories and no loss of agent performance.
