# Maintainer Docs

Maintain source quality, releases, discussions, and distribution with the operational guides below. Start with the [main field guide](../README.md) for Loop Engineering concepts and resources.

- [Curation standard](CURATION.md) - How resources are accepted, rejected, and replaced.
- [Maintenance guide](MAINTENANCE.md) - Weekly and monthly checks, link rot policy, and identity policy.
- [Distribution checklist](DISTRIBUTION.md) - Indexing, launch copy, and where to share.
- [Awesome submission checklist](AWESOME_SUBMISSION.md) - Upstream requirements and submission timing.
- [Outreach kit](OUTREACH.md) - Conservative messages for inviting corrections and patterns.
- [Discussion guide](DISCUSSIONS.md) - Discussion categories, starter prompts, and moderation standard.
